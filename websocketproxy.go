...
func (w *WebsocketProxy) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	ctx := req.Context()
	if w.Backend == nil {
		gotils.L(ctx).Error().Print("webproxy backend function is not defined")
	...
	if backendURL == nil {
		gotils.L(ctx).Error().Print("websocketproxy: backend URL is nil")
	...
	if err != nil {
		gotils.L(ctx).Error().Printf("websocketproxy:%s", err)
		if resp != nil {
			// If the WebSocket handshake fails, ErrBadHandshake is returned
			// along with a non-nil *http.Response so that callers can handle
			// redirects, authentication, etcetera.
			if err := copyResponse(rw, resp); err != nil {
				gotils.L(ctx).Error().Printf("websocketproxy: couldn't write response after failed remote backend handshake %s", err)
			}
		} else {
	...
	if err != nil {
		gotils.L(ctx).Error().Printf("websocketproxy: couldn't upgrade %s", err)
		return
	}
	defer connPub.Close()
	...
	replicateWebsocketConn := func(ctx context.Context, ip string, limit bool, dst, src *websocket.Conn, errc chan error) {
		for {
			...
			if limit && len(msg) > 0 {
				...
				if len(methods) > 0 {
					_, resp := w.Transport.block(ctx, res)
					if resp != nil {
						errc <- errors.New(resp.(ErrResponse).Error.Message)
						err = src.WriteMessage(websocket.CloseMessage, websocket.FormatCloseMessage(websocket.ClosePolicyViolation, resp.(ErrResponse).Error.Message))
						if err != nil {
							errc <- err
						}
						break
	...
	if e, ok := err.(*websocket.CloseError); !ok || e.Code == websocket.CloseAbnormalClosure {
		gotils.L(ctx).Error().Printf("%s %s", message, err)
	}
}
